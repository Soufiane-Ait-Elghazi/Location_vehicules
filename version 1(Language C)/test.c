...
Location *Louer()
{
    int Id_Vehicule, Id_Client;
    date date_debut, date_fin;
    char type_Payement[20], j[3], m[3], a[5], CIN[10];
        ...
        if (Client_etat_blocage(CIN) == 0)
        {
            if (Vehicule_disponible(Id_Vehicule) == 1)
            {
                ...
                do
                {
                    ...
                    gets(a);
                    j[2] = m[2] = a[4] = '\0';
                } while (strcmp(j, "01") < 0 || strcmp(j, "31") > 0 || strcmp(m, "01") < 0 || strcmp(m, "12") > 0 || strcmp(a, "2021") < 0);
                date_debut.j = atoi(j);
                date_debut.m = atoi(m);
                date_debut.a = atoi(a);
                L->dateDebut = date_debut;
                ...
                printf("Entrer le type du payement : ");
                gets(type_Payement);

                L->Id_Facture = AddFacture(Id_Client, type_Payement, 0, date_debut);
...
date localdate()
{
    ...
    date local;
    local.a = timeinfo->tm_year + 1900;
    local.m = timeinfo->tm_mon + 1;
    local.j = timeinfo->tm_mday;
